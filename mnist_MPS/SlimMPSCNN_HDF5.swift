...
//  mnist_MPS
//
//  Created by kazoo-kmt on 11/25/16.
//  Copyright © 2016 kazoo-kmt. All rights reserved.
//
//
/*
	Abstract:
	This file describes slimmer routines to create some common MPSCNNFunctions, it is useful especially to fetch network parameters from .h5 (HDF5) files
 This is based on SlimMPSCNN.swift provided by Apple.
 
 Note: This code is work in progress version and doesn't run because model parameters in HDF5 doesn't match the required input shape for MPS's convolution/fullyconnected layer's initialization. 
 */
...
class SlimMPSCNNConvolution: MPSCNNConvolution{
  ...
  init(kernelWidth: UInt, kernelHeight: UInt, inputFeatureChannels: UInt, outputFeatureChannels: UInt, neuronFilter: MPSCNNNeuron? = nil, device: MTLDevice, kernelParamsBinaryName: String, padding willPad: Bool = true, strideXY: (UInt, UInt) = (1, 1), destinationFeatureChannelOffset: UInt = 0, groupNum: UInt = 1){
    ...
    guard let group = file.openGroup(kernelParamsBinaryName) else {
      fatalError("Failed to open group of \(kernelParamsBinaryName)")
    }
    print(#line, group)
    guard let dataSet_w = group.openFloatDataset(kernelParamsBinaryName + "_W:0") else {
      fatalError("Failed to open data set of \(kernelParamsBinaryName)_W:0")
    }
    print(#line, dataSet_w)
    guard let dataSet_b = group.openFloatDataset(kernelParamsBinaryName + "_b:0") else {
      fatalError("Failed to open data set of \(kernelParamsBinaryName)_b:0")
    }
    print(#line, dataSet_b)
    guard let dataArray_w = try? dataSet_w.read() else {
      fatalError("Failed to read data set of \(kernelParamsBinaryName)_W:0")
    }
    print(#line, dataArray_w)
    guard let dataArray_b = try? dataSet_b.read() else {
      fatalError("Failed to read data set of \(kernelParamsBinaryName)_b:0")
    }
    print(#line, dataArray_b)
    ...
    super.init(device: device,
               ...
               kernelWeights: dataArray_w,
               biasTerms: dataArray_b,
...
class SlimMPSCNNFullyConnected: MPSCNNFullyConnected{
  ...
  init(kernelWidth: UInt, kernelHeight: UInt, inputFeatureChannels: UInt, outputFeatureChannels: UInt, neuronFilter: MPSCNNNeuron? = nil, device: MTLDevice, kernelParamsBinaryName: String, destinationFeatureChannelOffset: UInt = 0){
    ...
    /*
     w.deallocate(capacity: Int(inputFeatureChannels * kernelHeight * kernelWidth * outputFeatureChannels))
     b.deallocate(capacity: Int(outputFeatureChannels))
